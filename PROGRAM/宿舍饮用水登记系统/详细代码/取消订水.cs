...
namespace 宿舍饮用水登记系统
{
    public partial class 取消订水 : Form
    {
        ...
        public DataSet OrderNumber(NetworkStream ns, string account)   //查询当前订单号
        {
            //0表示验证---用户---用户名和密码是否正确
        ...
        public int Cancellation(string order)
        {
            //0表示验证---用户---用户名和密码是否正确
            //1表示---用户---返回窗口初始化信息(用户名和密码)
            //2表示---送水职工---返回窗口初始化信息（用户名，姓名，ip以及端口）
            ...
            //26表示---送水职工---查询职工密钥
            //27表示---送水职工---更改职工密钥
            AsciiGetBytesSend(ns, "16");
            AsciiGetBytesSend(ns, account);
            if(AsciiGetstringRead(ns)=="1")
            {
                AsciiGetBytesSend(ns, order);
                if (AsciiGetstringRead(ns) == "1")
                    return 1;
                else
                    return 0; 
        ...
        public 取消订水(Users user, NetworkStream ns, string account)
        {
            this.user = user;
            this.ns = ns;
            ...
            int count = OrderNumber(ns, account).Tables["Orders"].Rows.Count;
            for (int i = 0; i < count; i++)
            {
                MyItem rItem = new MyItem();
                rItem.Did = OrderNumber(ns, account).Tables["Orders"].Rows[i][0].ToString().TrimEnd() ;
                vtData.Add(rItem);
            }
        ...
        {
            int t = Cancellation(comboBox1.Text);
            if (t == 1)
                MessageBox.Show("取消订单成功！");
            else
                MessageBox.Show("取消订单失败，请重新取消！");
        }
    }
}
